...
// src/gap-chat/steering.ts
var DEFAULT_STEERING_PREFERENCES = {
  framework: "default",
  reflectionPoints: [],
  outputStyle: {
    length: "detailed",
...
var PREFERENCES_DIR = join(homedir(), ".summon", "preferences");
var TRACES_DIR = join(homedir(), ".summon", "traces");
...
function saveSteeringPreferences(ritualId, preferences) {
  ensurePreferencesDir();
  const path = getPreferencesPath(ritualId);
  try {
    const yaml = $stringify(preferences);
...
function mergePreferences(defaults, overrides) {
  return {
...
function loadExecutionTraces(ritualRef, limit = 10) {
  ensureTracesDir();
...
`).filter(Boolean);
    const traces = lines.slice(-limit).map((line) => JSON.parse(line));
    return traces.reverse();
  } catch {
    return [];
  }
}
function clearExecutionTraces(ritualRef) {
  ensureTracesDir();
  const normalizedRef = ritualRef.replace(/[@/]/g, "_");
  const traceFile = join(TRACES_DIR, `${normalizedRef}.jsonl`);
  if (existsSync(traceFile)) {
...
function parseSteeringCommand(input) {
  const trimmed = input.trim();
  if (trimmed.startsWith("SET_FRAMEWORK:")) {
    const framework = trimmed.split(":")[1]?.trim();
...
function createSteeringSession(sessionId, ritualId, preferences) {
  const session = {
    sessionId,
    ritualId,
...
function getSteeringSession(sessionId) {
  return activeSessions.get(sessionId);
}
function updateSteeringSession(sessionId, updates) {
...
function applySteeringCommand(preferences, command) {
...
function endSteeringSession(sessionId) {
  return activeSessions.delete(sessionId);
}
function getRitualId(ritual) {
  if (typeof ritual === "string") {
    return ritual;
  }
  return `${ritual.name}@${ritual.version}`;
}
function applyPreferencesToRitual(ritual, preferences) {
  const updated = JSON.parse(JSON.stringify(ritual));
  ...
  updated.persona.behavior.steeringPreferences = preferences;
  return updated;
}
